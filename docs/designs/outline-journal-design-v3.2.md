# Outline Journal（日记 + 日历）功能设计文档 v3.2

> v3.1 → v3.2 变更摘要（4 处）：
> 1. upsert 并发流程重写：锁后重查 entry，消除孤儿 Document 风险；唯一约束 catch 改为直接 throw 触发回滚
> 2. advisory lock key 改为 `hashtextextended(:key, 0)`，消除前 4 字节截断导致的 hash 冲突
> 3. 前端 modelName 改为 PascalCase `"JournalEntry"`，对齐 Outline 现有约定
> 4. 移除前端 `addPolicies(res.policies)` 调用（后端未返回 policies 字段）
>
