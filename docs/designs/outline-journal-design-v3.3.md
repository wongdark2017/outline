# Outline Journal（日记 + 日历）功能设计文档 v3.3

> v3.2 → v3.3 变更摘要（5 处）：
> 1. upsert 创建路径补 authorize(createCollection / createDocument) + rateLimiter
> 2. 后端 JournalEntry 模型补 @Fix 装饰器 + InferAttributes / InferCreationAttributes 泛型
> 3. __journal__ collection 在前端 CollectionsStore 中隐藏，新增核心改动 +1 处
> 4. 日期语义改为用户 timezone：前端用 Intl，后端 streak/calendar 接收 timezone 参数
> 5. 前端 JournalEntry 可序列化字段补 @Field 装饰器
>
...
app/stores/CollectionsStore.ts          # +1 行：侧栏集合列表过滤掉 __journal__ 系统集合（约 L39）
```

**关于 `__journal__` collection 的可见性：**

`__journal__` 集合的 `permission = null` 确保了其他团队成员看不到它，但创建者自己的侧栏集合列表会渲染它。Outline 前端的 `CollectionsStore` 在计算 `orderedData` 时只过滤了 `deletedAt` 和权限，没有"系统集合"的概念。

最小改动方案：在 `CollectionsStore` 的集合列表 getter 中加一行过滤：

```typescript
// app/stores/CollectionsStore.ts — 约 L39，orderedData 或类似 getter 中
.filter((c) => !c.name.startsWith("__"))
```

这是约定式隐藏（以 `__` 开头的集合视为系统集合），不需要给 Collection 模型加字段。如果未来有其他插件也需要系统集合，这个约定可以复用。
...
import { Collection, Document, JournalEntry, Team } from "@server/models";
...
router.post(
  ...
  async (ctx) => {
    ...
    // ── 权限校验 ──
    // 与 collections.create / documents.create 路由对齐，
    // 不绕过 Outline 的权限系统。
...
router.post(
  ...
  async (ctx) => {
    ...
    const streak = await calculateStreak(user.id, user.teamId, user.timezone);
...
export default class JournalEntriesStore extends Store<JournalEntry> {
   ...
   */
  private todayInUserTimezone(): string {
    const tz = this.rootStore.auth?.user?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone;
    const parts = new Intl.DateTimeFormat("en-CA", {    // en-CA 格式为 YYYY-MM-DD
      timeZone: tz,
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
    }).format(new Date());
    return parts;    // "2026-06-02"
  }
...
const CalendarGrid: React.FC<Props> = ({
  ...
  // 用户 timezone 的"今天"，由 Store 提供或用浏览器 Intl 推断
...
| `app/stores/CollectionsStore.ts` | 过滤 `__` 开头的系统集合 | +1 |
