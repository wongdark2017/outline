# Outline Journal（日记 + 日历）功能设计文档 v3.5

> v3.4 → v3.5 变更摘要（5 处）：
> 1. RateLimiterStrategy import 拆为 middleware/rateLimiter + utils/RateLimiter 两个路径
> 2. createDocument 授权前用 Collection.findByPk + userId 重载 membership scope，避免 invariant
> 3. createCollection 仅在 !collection 时校验，已有 journal collection 的用户不受团队策略变更影响
> 4. 修复 selectedDate 仍引用已删除的 todayInUserTimezone()，改为 constructor 中 this.today 赋值
> 5. __journal__ 过滤位置从 Sidebar/App.tsx 修正为 Sidebar/components/Collections.tsx
>
...
```typescript
// app/components/Sidebar/components/Collections.tsx — 约 L25，渲染 collections 列表处
const visibleCollections = collections.orderedData.filter(
