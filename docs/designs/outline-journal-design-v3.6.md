# Outline Journal（日记 + 日历）功能设计文档 v3.6

> v3.5 → v3.6 变更摘要（5 处边界收口）：
> 1. 侧栏过滤数据源从 orderedData 改为 allActive（避免 archived collection 泄漏）
> 2. 保留名保护扩展到 collections.update（防止重命名绕过）
> 3. timezone 严格统一：前端在 calendar 请求中传 timezone，后端 fallback 链为 request > user.timezone > UTC
> 4. collections.ts 文件路径修正为 server/routes/api/collections/collections.ts
> 5. Store 中 catch (err: any) 改为 catch (error) + instanceof Error 类型守卫
>
...
// 现有代码用 collections.allActive，不是 orderedData
const visibleCollections = collections.allActive.filter(
...
create 和 update 都要加，否则用户可以把普通集合重命名为 `__journal__`。
...
const calendarSchema = z.object({
  body: z.object({
    ...
    timezone: z.string().optional(),              // 前端传入，确保前后端一致
  }),
});
