# Outline Journal（日记 + 日历）功能设计文档 v3.7

> v3.6 → v3.7 变更摘要（6 处执行细节）：
> 1. 迁移文件扩展名 .ts → .js（migration runner 只匹配 *.js）
> 2. timezone schema 从 z.string() 改为 zodTimezone()（防止无效 timezone 炸服务端）
> 3. 保留名检查改为 name?.trim().startsWith("__")（update 中 name 是 optional）
> 4. 侧栏过滤变量名改为 orderedCollections（对齐真实组件的拖拽排序和渲染变量）
> 5. Journal Scene mount 时加 fetchRange 最近 14 天（否则 RecentEntries 首次为空）
> 6. migration down 补 DROP TYPE IF EXISTS "enum_journal_entries_mood"
>
...
const Journal = observer(() => {
  ...
  useEffect(() => {
    ...
    journalEntries.fetchRange(start, end);
