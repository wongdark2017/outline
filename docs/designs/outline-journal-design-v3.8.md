# Outline Journal（日记 + 日历）功能设计文档 v3.8

> v3.7 → v3.8 变更摘要（5 处）：
> 1. Processor 只监听 documents.permanent_delete；软删除保留 JournalEntry 以支持 Trash → Restore
> 2. 日期 schema 从纯正则改为 zodDateOnly()：正则 + refine 校验真实日期，拒绝 "2026-02-31"
> 3. Journal Scene 中 async 调用加 void 前缀，避免 no-floating-promises lint error
> 4. RootStore / Event 改为 import type，避免 consistent-type-imports lint error
> 5. 侧栏示例补 CalendarIcon import 声明
>
