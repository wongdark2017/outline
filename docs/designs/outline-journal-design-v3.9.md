# Outline Journal（日记 + 日历）功能设计文档 v3.9

> v3.8 → v3.9 变更摘要（4 处）：
> 1. upsert/info 处理 "Document in Trash"：upsert 返回 409 提示恢复，info 返回 404
> 2. Store.fetchRange 改为 reconcile：移除该范围内 API 不再返回的 entry，防止已软删除日记残留
> 3. journal.entries schema 加 startDate <= endDate refine + 366 天范围上限
> 4. 测试运行清单补 collections.test.ts；测试用例补 Trash 状态 + 日期范围校验
>
