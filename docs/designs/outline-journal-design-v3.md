# Outline Journal（日记 + 日历）功能设计文档 v3

> v2 → v3 变更摘要：
> - 修正 Hook.Processor 支持判断：Document 删除联动改为插件 processor，核心改动从 6 处减为 5 处
> - journal.upsert 重写为 Outline 实际路由风格：auth() + validate() + transaction() middleware 链
> - documentCreator 调用签名改为 (ctx, props)
> - 前端 Model 改为继承 ./base/Model，Store 改为继承 ./base/Store\<T\>
> - Sidebar 接入点精确到 app/components/Sidebar/App.tsx
> - 删除未使用的 IsDate import，修正 IdModel 为默认导出
...
app/stores/RootStore.ts                 # +2 行：JournalEntriesStore 注册（约 L40 附近）
```
...
export default class JournalProcessor {
  /**
   * Document 软删除时，同步删除关联的 JournalEntry。
   *
   * 注意：Outline 的 Document 删除流程是先软删除（设 deletedAt），
   * 后台任务再物理删除。外键 CASCADE 只在物理删除时触发。
   * 这里处理软删除阶段，确保日历上不再显示已删除的日记。
   */
...
```typescript
// app/stores/RootStore.ts — 新增 import + 初始化
...
| `app/stores/RootStore.ts` | import + 初始化 JournalEntriesStore | +2 |
