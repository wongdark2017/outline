# Outline Journal（日记 + 日历）功能设计文档 v4.2

> v4.1 → v4.2 变更摘要（5 处）：
> 1. journal.entries 的 Document attributes 补 `urlId`（否则 document.url 生成 `/doc/title-undefined`）
> 2. 前端测试文件改为与源码共置：`index.test.tsx` / `Collections.journal.test.tsx`，不建 `__tests__` 目录
> 3. Store 拆分 `error`（后台加载）和 `actionError`（用户操作），Scene 仅 toast actionError
> 4. handleSelectDate 写回 `journalEntries.selectedDate`，409/失败时日历选中态正确更新
> 5. Scene 补 `LoadingIndicator` import + `StreakInfo` styled-component 定义
>
