# Outline Journal（日记 + 日历）功能设计文档 v4.3

> v4.2 → v4.3 变更摘要（4 处）：
> 1. JournalProcessor 类型修正：`DocumentEvent` 替代 `Event`，`TEvent["name"][]` 替代 `string[]`
> 2. upsert 后端拒绝未来日期（400），与前端 CalendarGrid isFuture 一致
> 3. calculateStreak 从逐日 N+1 查询改为单次批量查询最近 366 天 + 内存计算
> 4. 执行步骤补 collections 保留名测试 + 前端测试文件
>
...
router.post(
  ...
  async (ctx) => {
    ...
    // ── 未来日期检查 ──
    // UI 层已禁止点击未来日期，但 API 和 /journal/:date URL 可绕过。
    // 用用户 timezone 判断，与前端 CalendarGrid 的 isFuture 逻辑一致。
...
async function calculateStreak(userId: string, teamId: string, timezone: string): Promise<number> {
  ...
  // 避免逐日 count 的 N+1 问题（连续写几年会产生上千次查询）。
  const lookbackStart = addDays(today, -365);
