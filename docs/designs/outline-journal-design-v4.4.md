# Outline Journal（日记 + 日历）功能设计文档 v4.4

> v4.3 → v4.4 变更摘要（3 处）：
> 1. upsert schema + 前端请求补 timezone 参数，后端 fallback 链与 calendar 统一：request > user.timezone > UTC
> 2. streak 366 上限明确化：函数注释说明 MVP 语义，前端显示 "366+"，补测试用例
> 3. presenter import 改为 type-only
>
...
router.post(
  ...
  async (ctx) => {
    ...
    // ── 未来日期检查 ──
    // timezone 优先级与 calendar 一致：request > user.timezone > UTC
    const userTz = timezone ?? user.timezone ?? "UTC";
...
async function calculateStreak(userId: string, teamId: string, timezone: string): Promise<number> {
  ...
  // 单次查询最近 366 天内有日记的日期，在内存中计算 streak。
  // MVP 上限 366 天（覆盖完整一年）。如果 streak 超过 366，
  // 返回 366 并由前端显示为 "366+"。
  // 后续如果需要精确无上限 streak，可改为按 366 天窗口
  // 批量向前查直到找到断点。
  const MAX_STREAK_LOOKBACK = 366;
  const lookbackStart = addDays(today, -(MAX_STREAK_LOOKBACK - 1));
