# Outline Journal（日记 + 日历）功能设计文档 v4.5

> v4.4 → v4.5 变更摘要（3 处）：
> 1. streak 改为 367 天哨兵查询 + `{ streak, capped }` 返回值，精确 366 不再误标 "366+"
> 2. upsert timezone 优先级改为 `user.timezone ?? request ?? UTC`（mutation 以 profile 为准，不可客户端覆盖）
> 3. migration 补 `documentId` 索引（processor 按 documentId 删除时需要）
>
