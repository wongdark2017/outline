# Outline Journal（日记 + 日历）功能设计文档 v4.6

> v4.5 → v4.6 变更摘要（3 处）：
> 1. `calculateStreak` 返回类型从 `Promise<number>` 改为 `Promise<{ streak: number; capped: boolean }>`
> 2. upsert 测试补 timezone 优先级用例（user.timezone 存在时不可被 request 覆盖）
> 3. streak 测试补精确 366 vs 367 边界用例（防止回归到 >= 366 的旧 bug）
>
