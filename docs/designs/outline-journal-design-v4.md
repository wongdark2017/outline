# Outline Journal（日记 + 日历）功能设计文档 v4.0

> v3.9 → v4.0 变更摘要（5 处）：
> 1. Journal Scene 展示 409 Trash 错误：通过 toast 提示用户去 Trash 恢复文档
> 2. Migration up/down 包事务：防止中途失败留下半完成 schema
> 3. 366 天范围校验改为 inclusive day count，消除 off-by-one
> 4. CalendarGrid 改为函数声明，移除 React import，对齐仓库风格
> 5. 测试章节去重 + 运行命令补前端测试
>
...
const Journal = observer(() => {
  ...
  useEffect(() => {
    if (journalEntries.error) {
      showToast(journalEntries.error, { type: "warning" });
...
# 前端组件测试
yarn test app/scenes/Journal
yarn test app/components/Sidebar/components/Collections.test.ts
```
