# Outline Journal（日记 + 日历）功能设计文档 v5.2

> v5.1 → v5.2 变更摘要（3 处）：
> 1. APIContext 泛型从 `typeof schema`（ZodObject 类型）改为 `z.infer<typeof schema>`（解析后的 request 类型）
> 2. update/delete policy 增加 `!actor.isViewer && !actor.isGuest`，read 仅校验 owner/team
> 3. 测试补"不复用公开同名集合" + "viewer/guest 无法更新 mood/tags"用例
>
