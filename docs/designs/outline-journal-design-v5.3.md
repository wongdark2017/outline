# Outline Journal（日记 + 日历）功能设计文档 v5.3

> v5.2 → v5.3 变更摘要（3 处）：
> 1. 侧栏过滤条件对齐后端：只隐藏 `name === "__journal__" && permission === null && sharing === false` 的集合
> 2. 分享策略明确化：`sharing: false` 禁止公开链接分享，这是有意设计；用户可移动文档后分享
> 3. 集合查找补 `archivedAt: null`，避免命中已归档集合导致 createDocument 授权失败
>
...
// 只隐藏 Journal 私有系统集合，不误隐藏用户遗留的公开同名集合
const orderedCollections = collections.allActive.filter(
  (c) =>
    !(c.name === "__journal__" && c.permission === null && c.sharing === false)
);
```
...
```
- __journal__ 私有集合不出现在侧栏集合列表中
- 公开的 __journal__ 同名集合不被误隐藏
- 普通名称的集合不被误过滤
