# Outline Journal（日记 + 日历）功能设计文档 v5.4

> v5.3 → v5.4 变更摘要（1 处）：
> 1. Journal 系统集合改用 `sourceMetadata: { externalId: "journal" }` 标记识别，替代 name/permission/sharing 三重组合。后端查找、创建、侧栏过滤全部基于 sourceMetadata，彻底消除历史同名集合误判。
>
...
Journal 系统集合通过 `sourceMetadata.externalId === "journal"` 标记识别，而非靠名称/权限/sharing 组合。这彻底消除了历史同名集合的误判风险——即使用户上线前已有同名且恰好私有的 `__journal__` 集合，只要它没有 `sourceMetadata.externalId === "journal"`，就不会被复用或隐藏。
...
const orderedCollections = collections.allActive.filter(
  (c) => c.sourceMetadata?.externalId !== "journal"
);
```
  ...
  async (ctx: APIContext<UpsertReq>) => {
    ...
    let collection = await Collection.findOne({
      where: {
        ...
        createdById: user.id,
        archivedAt: null,
        "sourceMetadata.externalId": "journal",
      },
      transaction,
    });
...
describe("journal.upsert", () => {
  ...
  it("should create collection with sourceMetadata.externalId = 'journal'");
...
```
- Journal 系统集合（sourceMetadata.externalId = "journal"）不出现在侧栏
