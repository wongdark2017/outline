# Outline Journal（日记 + 日历）功能设计文档 v5.5

> v5.4 → v5.5 变更摘要（4 处）：
> 1. 前端 `app/models/Collection.ts` 补 `sourceMetadata` 属性声明（不加 @Field）
> 2. `externalId` 改为 `"outline:journal"` 命名空间前缀，避免与导入流程的 externalId 碰撞
> 3. 测试补正向用例："应复用已标记 sourceMetadata 的 Journal 集合"
> 4. 核心文件改动从 7 处更新为 8 处，执行步骤补 Collection.ts
>
