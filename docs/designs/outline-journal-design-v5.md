# Outline Journal（日记 + 日历）功能设计文档 v5.0

> v4.6 → v5.0 变更摘要（2 处收口）：
> 1. upsert 已有 entry 分支补 `authorize(user, "update", entry)`，对齐 Outline policy 使用习惯
> 2. 前端测试补 streakCapped 展示逻辑用例（366 精确 vs 366+ 截断 vs 0 不渲染）
>
> **v5.0 为最终可执行版本。**
>
