# Outline Journal（日记 + 日历）功能设计文档 v6.1

> v6.0 → v6.1 变更摘要（5 处）：
> 1. mood schema 改为 `.nullable().optional()`，支持传 `null` 清除心情
> 2. 前端 JournalEntry 删除 `id/createdAt/updatedAt` 重复声明（已在基类 Model 中）
> 3. CalendarGrid 去掉不完整的 `role="grid"`，保留原生 button 无障碍；NavButton 补 `type="button"`
> 4. 导出函数/类补完整 JSDoc（@param/@returns + 类描述）
> 5. 测试补 tags 空串/超长/超量拒绝、mood 清除、a11y 属性断言
>
> **v6.1 为执行稿。**
>
