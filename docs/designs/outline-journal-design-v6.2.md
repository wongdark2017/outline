# Outline Journal（日记 + 日历）功能设计文档 v6.2

> v6.1 → v6.2 变更摘要（3 处 lint/规范收口）：
> 1. 单行 if 全部补花括号（`curly: error`）
> 2. tags 字段补 `allowNull: false`（模型 + migration），防止 DB 出现 null 打破类型
> 3. server/client JournalEntry 模型类 + CalendarGrid 补完整 JSDoc
>
> **v6.2 为执行稿。**
>
...
app/components/Sidebar/App.tsx                     # +3 行：Journal 侧栏链接 + CalendarIcon import
app/components/Sidebar/components/Collections.tsx  # +2 行：过滤 Journal 系统集合（sourceMetadata）
app/stores/RootStore.ts                            # +2 行：import + registerStore(JournalEntriesStore)
app/models/Collection.ts                           # +3 行：声明 sourceMetadata 属性（约 L19）
server/routes/api/collections/collections.ts       # +4 行：create + update 禁止 __ 开头的保留名
```

`app/models/Collection.ts` 需要声明 `sourceMetadata` 字段，否则侧栏过滤中 `c.sourceMetadata?.externalId` 会是 TypeScript 类型错误：

```typescript
// app/models/Collection.ts — 约 L19，在现有属性声明中新增

@observable.shallow
sourceMetadata?: { externalId?: string; externalName?: string } | null;
```

服务端 Collection 模型和 presenter 已有该字段（`@Column(DataType.JSONB) sourceMetadata`），前端只是缺少对应的属性声明。不加 `@Field`——这不是用户通过 collection update 编辑的字段。

**关于 `__journal__` collection 的可见性：**

Journal 系统集合通过 `sourceMetadata.externalId === "outline:journal"` 标记识别。使用 `outline:` 命名空间前缀避免与导入流程的 `externalId` 碰撞。即使用户上线前已有同名集合，只要它没有这个标记就不会被复用或隐藏。

侧栏过滤基于同一标记：

```typescript
// app/components/Sidebar/components/Collections.tsx — 约 L29
// 用 sourceMetadata 标记识别 Journal 系统集合，
// 不再依赖 name/permission/sharing 组合，避免历史同名集合误判
const orderedCollections = collections.allActive.filter(
  (c) => c.sourceMetadata?.externalId !== "outline:journal"
);
```
...
/**
 * A journal entry linking a date to a Document within a user's
 * private Journal collection. Tracks optional mood and tags metadata.
 */
@Fix
...
JournalEntry 元数据（mood、tags）仅作者可访问。

Journal collection 的 `sharing: false` 会禁止该集合下文档的公开链接分享。这是 MVP 的有意设计——日记是个人私有内容，不应支持生成公开链接。

用户如需分享日记正文，可将文档移动到其他集合后再分享（脱离 Journal 上下文），或复制内容到其他文档。
...
const upsertSchema = z.object({
  body: z.object({
    date: zodDateOnly(),
    mood: z.enum(["productive", "neutral", "tired", "inspired", "frustrated"]).nullable().optional(),
...
router.post(
  ...
  async (ctx: APIContext<UpsertReq>) => {
    ...
    //    用 sourceMetadata.externalId 标记 Journal 系统集合，
    //    不再依赖 name/permission/sharing 组合判断。
    //    这样即使用户上线前已有同名私有集合也不会误复用。
    let collection = await Collection.findOne({
      where: {
        teamId: user.teamId,
        createdById: user.id,
        archivedAt: null,
        "sourceMetadata.externalId": "outline:journal",
      },
      transaction,
    });
...
/**
 * Handles cleanup of JournalEntry records when their associated
 * Document is permanently deleted from the system.
 */
...
/** Client-side model for journal entries, managed by JournalEntriesStore. */
class JournalEntry extends Model {
  static modelName = "JournalEntry";

  store: JournalEntriesStore;

  // id, createdAt, updatedAt は基类 Model で宣言済み
...
/**
 * Manages journal entries, calendar data, and streak tracking.
 * API endpoints use the "journal.*" namespace.
 */
...
/**
 * Monthly calendar grid for the Journal feature.
 * Renders day buttons with entry indicators, today highlight,
 * and accessible navigation controls.
 *
 * @param props - Calendar display and interaction props
 * @returns Calendar grid with month navigation
 */
function CalendarGrid({
  ...
  return (
    <Wrapper aria-label="Journal calendar">
      <MonthNav>
        <NavButton type="button" onClick={handlePrev} aria-label="Previous month">&lsaquo;</NavButton>
        ...
        <NavButton type="button" onClick={handleNext} aria-label="Next month">&rsaquo;</NavButton>
      </MonthNav>
        ...
        {cells.map((cell, i) =>
          cell ? (
            <DayButton
              ...
              aria-label={`${cell.date}${cell.hasEntry ? ", has entry" : ""}`}
...
const DayButton = styled.button<{
  ...
  &:hover:not(:disabled) {
    background: ${(p) => (p.$isSelected ? s("accent") : s("secondaryBackground"))};
  }
`;
...
describe("journal.upsert", () => {
  ...
  // v5.2 新增
  it("should not reuse a public __journal__ collection, create new private one instead");
  it("should not reuse a private __journal__ collection without sourceMetadata.externalId");
  it("should create collection with sourceMetadata.externalId = 'outline:journal'");
  it("should reuse existing collection with outline:journal sourceMetadata marker");
  it("should allow viewer to open existing entry without mood/tags (read)");
  it("should reject viewer updating mood/tags (update denied)");
  it("should reject guest updating mood/tags (update denied)");
  ...
  it("should reject tags exceeding 40 characters");
  it("should reject more than 20 tags");
  it("should clear mood when mood is null");
  // v4.6 新增
...
```
- Journal 系统集合（sourceMetadata.externalId = 'outline:journal'）不出现在侧栏
- 同名 __journal__ 集合若无 sourceMetadata 不被误隐藏
- 普通名称的集合不被误过滤
- CalendarGrid 的 today 高亮与 selectedDate 独立工作
- 未来日期按钮为 disabled 状态
- 今天的日期按钮带 aria-current="date"
- 选中日期按钮带 aria-pressed="true"
- 每个日期按钮有描述性 aria-label
...
| `app/components/Sidebar/App.tsx` | Journal 侧栏链接 + CalendarIcon import | +3 |
| `app/components/Sidebar/components/Collections.tsx` | 过滤 Journal 系统集合（sourceMetadata） | +2 |
| `app/stores/RootStore.ts` | import + registerStore(JournalEntriesStore) | +2 |
| `app/models/Collection.ts` | 声明 sourceMetadata 属性 | +3 |
...
12. `app/models/Collection.ts` — 补 `sourceMetadata` 属性声明
13. `app/stores/JournalEntriesStore.ts` + RootStore 注册
14. `app/scenes/Journal/` 全部组件
15. `authenticated.tsx` 路由注册
16. `Sidebar/App.tsx` 入口 + `Sidebar/components/Collections.tsx` 过滤
17. `app/scenes/Journal/index.test.tsx` — Journal Scene 测试
18. `app/components/Sidebar/components/Collections.journal.test.tsx` — 集合过滤测试
