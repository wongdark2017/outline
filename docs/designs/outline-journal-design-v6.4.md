# Outline Journal（日记 + 日历）功能设计文档 v6.4

> v6.3 → v6.4 变更摘要（2 处规范收口）：
> 1. advisory lock 改用 `import { sequelize } from "@server/storage/database"`，去掉 `!` 非空断言
> 2. JSDoc `@param`/`@returns` 统一小写开头 + 句号结尾
>
> **v6.4 为最终执行稿。**
>
...
class JournalEntry extends Model {
  ...
  // id, createdAt, updatedAt are declared in base Model
