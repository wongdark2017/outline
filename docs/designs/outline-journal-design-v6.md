# Outline Journal（日记 + 日历）功能设计文档 v6.0

> v5.5 → v6.0 变更摘要（4 处实现质量改进）：
> 1. tags schema 加 `.trim().min(1).max(40)` 单项限制 + `.max(20)` 数组上限
> 2. CalendarGrid 改为 `<button>` + `disabled` + `aria-current/pressed/label`，月份导航补 `aria-label`
> 3. 组件状态变更改为调用 Store action：`setSelectedDate()` / `setCurrentMonth()`
> 4. presenter 补 JSDoc
>
> **v6.0 为执行稿。**
>
...
function CalendarGrid({
  ...
  return (
    <Wrapper role="grid" aria-label="Journal calendar">
      <MonthNav>
        <NavButton onClick={handlePrev} aria-label="Previous month">&lsaquo;</NavButton>
        ...
        <NavButton onClick={handleNext} aria-label="Next month">&rsaquo;</NavButton>
      </MonthNav>

      <WeekdayRow role="row">
        {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((d) => (
          <WeekdayHeader key={d} role="columnheader">{d}</WeekdayHeader>
        ))}
      </WeekdayRow>
