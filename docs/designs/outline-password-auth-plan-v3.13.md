# Outline 邮箱 + 密码登录功能 — 技术方案 v3.13

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.13(十五轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.11.md`。本稿为独立全文,以本稿为准。
...
- 未配置 SMTP 时:端点返回明确的 unavailable 错误,前端隐藏"忘记密码"入口,改走 §6。**服务端判断口径**(v3.12):`env.EMAIL_ENABLED && !!env.SMTP_FROM_EMAIL`——`EMAIL_ENABLED` 只检查 `SMTP_HOST || SMTP_SERVICE || development`(`server/env.ts:401`),但 `BaseEmail.schedule()` 在 `SMTP_FROM_EMAIL` 缺失时直接 no-op(`server/emails/templates/BaseEmail.tsx:58`),即 SMTP 连接可用但无发件人时邮件静默不发。服务端必须同时检查两者,任一缺失即返回 unavailable 错误。**前端判断口径**:前端只能读 `@Public` 的 `EMAIL_ENABLED`(无法读 `SMTP_FROM_EMAIL`),因此"忘记密码"入口可见性仍用 `env.EMAIL_ENABLED`;若 `SMTP_FROM_EMAIL` 缺失导致服务端 unavailable,前端展示错误提示(§7.1 的"忘记密码"子状态需准备 unavailable 错误展示,不能假定恒 success)。
...
- 鉴权**必须且只能其一**(schema 层 `.superRefine()` 强制 XOR,两者都缺失或同时出现都在 validation 阶段 400,见 §5.0):
  1. **reset token 路径(消费顺序写死)**:请求体字段为 **`resetToken`**(理由见 §5.0 红线;schema 按 §5.0 `.strict()` 枚举,`token` 等未知字段在 validation 阶段 400)。验证参照 `getUserForEmailSigninToken` 的形态(`server/utils/jwt.ts:99`):
     ...
     5. 校验 Redis 中 `jti` **存在性**(`GET`,不消费),并核对 value 中的 `{ userId, teamId }` 与 payload 一致;jti 不存在即拒绝(已消费或从未生成)。**GETDEL 统一在后续事务块内执行**(见下文"jti 消费时序"),此步只做存在性校验,不消费。
...
- **事务要求(v3.10)**:update handler 的落库操作涉及多步写入(`setPassword` → `save` → `rotateJwtSecret` → 写事件),必须在同一事务内完成——半截失败(如 `rotateJwtSecret` 成功但事件写入失败)会导致状态不一致。`/auth` app **不像 `/api` 有全局 `transaction()` 中间件**(`server/routes/auth/index.ts` 的链路只有 bodyParser → coalesceBody → verifyCSRFToken → router),handler 内部必须**显式使用 `sequelize.transaction()`**。同时注意 **`User.rotateJwtSecret` 的真实签名是 `(options: SaveOptions)`**(内部 `this.save(options)`,`server/models/User.ts:598`),不是无参调用——必须传 `{ transaction }`。参照 `auth.delete` 端点的做法(`server/routes/api/auth/auth.ts:188`:该端点通过 `transaction()` 中间件获取事务,再 `user.rotateJwtSecret({ transaction })`)。update handler 骨架:
  ...
  await sequelize.transaction(async (transaction) => {
    ...
    // ── jti 消费(仅 reset token 路径) ──
    // 验证链第 5 步只做了 GET 存在性校验,此处在事务内执行 GETDEL 真正消费。
    if (isResetTokenPath) {
      await redisGetDel(jtiKey); // 事务回滚后 token 已消耗,见下文 tradeoff
    }
  ...
  if (isResetTokenPath) {
    ...
    // (ApiClient 对 2xx 会 response.json(),302/HTML 会导致设置页解析失败)
    await resignSession(ctx, user); // 等价 signIn 的 cookie 部分
    ctx.body = { success: true };
  }
  ```

  > **jti 消费时序(v3.12)**:Redis `GETDEL` 与 DB 事务不在同一个原子边界内,存在不一致窗口。两种策略:
  >
  > 1. **GETDEL 放在事务内、事务提交前**(推荐):若事务回滚,token 已被消耗——用户需重新发起 reset。这是**更安全的方向**(宁可让合法用户多发一次 reset,也不让已消费的 token 在回滚后"复活"被重放)。
  > 2. GETDEL 放在事务提交后:回滚时 token 不消耗,用户可重试;但如果提交成功而 GETDEL 失败(极端),token 存在重放窗口。
  >
  > 首版采用策略 1。§9 测试 8 的"二次使用被拒"用例已覆盖;补测试 19b"事务回滚后 token 已消耗,用户需重新申请"。

  > 登录态路径的重签会话与 JSON 响应都在事务提交**后**进行(签 cookie 是 HTTP 响应操作,不是 DB 写入)。reset 路径只做 302 redirect,无需 cookie 操作。
...
- **改密成功后的会话处理(保守路径)**:事务内完成 `setPassword()` + `rotateJwtSecret({ transaction })` 后:
  ...
  - **登录态路径**:`rotateJwtSecret()` 会使当前会话同样失效——事务提交后当场重签 cookie + 返回 JSON `{ success: true }`(设置页 RPC 调用经 `ApiClient`,对 2xx 会 `response.json()`,不能返回 302/HTML)。
...
> **CLI 落库要求(v3.12)**:CLI 脚本必须与服务端 update handler 使用**同一套落库语义**,在 `sequelize.transaction()` 内完成:
>
...
- [ ] **Redis jti GETDEL 放在事务内、事务提交前**(§5.3):接受"事务回滚后 token 已消耗,用户需重新发起 reset"的 tradeoff,防止回滚后 token 复活被重放。
...
12. **SMTP 不可用时 reset 端点拒绝(两个子用例)**:
    ...
    b. `EMAIL_ENABLED = true` 但 `SMTP_FROM_EMAIL` 缺失时,`/auth/password/reset` 返回 `ServiceUnavailableError`(503);前端"忘记密码"子状态展示错误提示(入口可见但提交失败,因为前端只能读 `EMAIL_ENABLED`);
...
19. **(v3.12)reset token 审计事件归属与 auth context 隔离**:
...
20. **(v3.12)CLI set-password 落库完整性**:CLI 脚本重置密码后,旧 cookie 失效(`jwtSecret` 已轮转);`failedSignInAttempts` 清零;`lockedUntil` 清空;`users.update` 审计事件存在且 `data.viaCliScript === true`;用新密码登录正常。
21. **(v3.13)设置页改密 RPC 响应形态**:登录态路径 POST `/auth/password/update`(带 `currentPassword` + `password`,RPC 经 `ApiClient`)成功时返回 JSON `{ success: true }` + 重签 cookie,**不返回 302/HTML**;前端无跳转,在设置页原地提示成功;reset token native form 路径仍返回 302(§5.3);两条路径的响应形态不能混淆。
