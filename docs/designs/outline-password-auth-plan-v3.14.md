# Outline 邮箱 + 密码登录功能 — 技术方案 v3.14

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.14(十六轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.12.md`。本稿为独立全文,以本稿为准。
...
- 鉴权**必须且只能其一**(schema 层 `.superRefine()` 强制 XOR,两者都缺失或同时出现都在 validation 阶段 400,见 §5.0):
  ...
  2. **登录态路径**:必须满足 `ctx.state.auth.user` 存在**且** `ctx.state.auth.type === AuthenticationType.APP`(cookie/session 认证);`/auth` provider 路由前置的 `authMiddleware({ optional: true })` 会接受 OAuth header(`AuthenticationType.OAUTH`,`:163`)、API key(`AuthenticationType.API`,`:205`)、APP JWT(`:243`)等多种认证来源——改密操作**只接受 APP session**,其他认证类型一律拒绝(401)。校验通过后再 `verifyPassword(currentPassword)`,通过才允许改。
- 密码策略:最少 12 位。
...
- **事务要求(v3.10)**:update handler 的落库操作涉及多步写入(`setPassword` → `save` → `rotateJwtSecret` → 写事件),必须在同一事务内完成——半截失败(如 `rotateJwtSecret` 成功但事件写入失败)会导致状态不一致。`/auth` app **不像 `/api` 有全局 `transaction()` 中间件**(`server/routes/auth/index.ts` 的链路只有 bodyParser → coalesceBody → verifyCSRFToken → router),handler 内部必须**显式使用 `sequelize.transaction()`**。同时注意 **`User.rotateJwtSecret` 的真实签名是 `(options: SaveOptions)`**(内部 `this.save(options)`,`server/models/User.ts:598`),不是无参调用——必须传 `{ transaction }`。参照 `auth.delete` 端点的做法(`server/routes/api/auth/auth.ts:188`:该端点通过 `transaction()` 中间件获取事务,再 `user.rotateJwtSecret({ transaction })`)。update handler 骨架:
  ...
  await sequelize.transaction(async (transaction) => {
    ...
    // ── jti 原子消费(仅 reset token 路径,权威判定) ──
    ...
    if (isResetTokenPath) {
      ...
      if (jtiData.userId !== payload.id || jtiData.teamId !== payload.teamId) {
        throw new Error("Reset token payload mismatch");
      }
    }
    ...
    if (isResetTokenPath) {
      ...
      await Event.createFromContext(
        ctx,
        { name: "users.update", userId: user.id, data: { passwordChanged: true } },
  ...
  if (isResetTokenPath) {
    ...
    ctx.cookies.set("accessToken", user.getSessionToken(expires, "password"), {
      sameSite: "lax",
      expires,
      domain,
    });
  ...
  > **jti 消费时序(v3.14)**:Redis `GETDEL` 是**唯一的一次性消费操作**,放在事务内、事务提交前。不再先 GET 后 GETDEL——两阶段方案存在并发绕过(两个请求都通过 GET 后,第二个 GETDEL 返回 null 但若不检查返回值仍会继续改密)。单次 GETDEL 以返回值为权威判定:非 null 即首次消费成功,null 即拒绝。
  >
  > **回滚 tradeoff**:若事务回滚,token 已被 GETDEL 消耗——用户需重新发起 reset。这是**更安全的方向**(宁可让合法用户多发一次 reset,也不让已消费的 token 在回滚后"复活"被重放)。§9 测试 19b 覆盖此场景。

  > 登录态路径的 cookie 重签与 JSON 响应都在事务提交**后**进行(签 cookie 是 HTTP 响应操作,不是 DB 写入;不调用 `signIn`——`signIn` 会 `updateSignedIn`、写 `users.signin` 事件并 redirect,这里只需要 cookie)。reset 路径只做 302 redirect,无需 cookie 操作。
...
- **改密成功后的会话处理(保守路径)**:事务内完成 `setPassword()` + `rotateJwtSecret({ transaction })` 后:
  ...
  - **登录态路径**:`rotateJwtSecret()` 会使当前会话同样失效——事务提交后重签 cookie:`user.getSessionToken(expires, "password")` → 设置 `accessToken` cookie(sameSite lax,3 个月过期)→ 返回 JSON `{ success: true }`。**不调用 `signIn`**(`signIn` 会 `updateSignedIn`、写 `users.signin` 事件并 redirect,`server/utils/authentication.ts:70-137`,改密不应产生这些副作用);不产生 `users.signin` 审计事件。**认证边界**:`currentPassword` 路径只接受 `AuthenticationType.APP`(cookie/session),API key / OAuth token 等其他认证类型一律 401(§5.3)。
...
- [ ] **登录态改密路径只接受 APP session**:`currentPassword` 路径必须 `ctx.state.auth.user` 存在且 `ctx.state.auth.type === AuthenticationType.APP`;API key / OAuth token / header/body/query token 一律 401。
...
- [ ] 改密成功调用 `rotateJwtSecret({ transaction })` 吊销全部旧会话(含未消费的 reset token);登录态路径事务提交后当场重签或要求重登。
...
19. **reset token 审计事件归属与 auth context 隔离**:
    ...
    b. **事务回滚后 token 已消耗**:模拟事务内 `rotateJwtSecret` 抛错导致回滚,密码未改;同一 reset token 再次使用被拒(jti 已被 GETDEL 消耗);用户需重新发起 reset;
...
21. **(v3.13/v3.14)设置页改密 RPC 响应形态**:登录态路径 POST `/auth/password/update`(带 `currentPassword` + `password`,RPC 经 `ApiClient`)成功时返回 JSON `{ success: true }` + 重签 cookie,**不返回 302/HTML**;前端无跳转,在设置页原地提示成功;旧 cookie 失效,新 cookie 可用;**不产生 `users.signin` 审计事件**(改密不是登录);reset token native form 路径仍返回 302(§5.3);两条路径的响应形态不能混淆。
22. **(v3.14)reset token 并发双提交**:同一 reset token 同时发起两次 POST `/auth/password/update`,只有一次成功改密,另一次因 GETDEL 返回 null 被拒绝;不允许两次都成功;此用例用于钉死单次 GETDEL 权威判定——若实现者误用先 GET 后 GETDEL 的两阶段方案,并发请求可能双双通过 GET。
23. **(v3.14)登录态改密认证边界**:
    a. 无 auth(匿名)POST `/auth/password/update` 带 `currentPassword` → 401;
    b. API key(`AuthenticationType.API`)POST 带 `currentPassword` → 401;
    c. OAuth token(`AuthenticationType.OAUTH`)POST 带 `currentPassword` → 401;
    d. header/body/query 传 token(非 cookie)POST 带 `currentPassword` → 401 或 400(§5.0 红线);
    e. 仅 cookie session(`AuthenticationType.APP`)+ 正确 `currentPassword` → 成功(200 JSON)。
...
| 测试(21 项,CSRF/schema 六子项,含并发计数、auth context 隔离、RPC 响应形态与 cloud guard)与安全打磨 | 2 – 3 天 |
