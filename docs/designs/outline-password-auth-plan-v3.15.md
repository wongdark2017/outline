# Outline 邮箱 + 密码登录功能 — 技术方案 v3.15

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.15(十七轮代码评审后)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.13.md`。本稿为独立全文,以本稿为准。
...
- **事务要求(v3.10)**:update handler 的落库操作涉及多步写入(`setPassword` → `save` → `rotateJwtSecret` → 写事件),必须在同一事务内完成——半截失败(如 `rotateJwtSecret` 成功但事件写入失败)会导致状态不一致。`/auth` app **不像 `/api` 有全局 `transaction()` 中间件**(`server/routes/auth/index.ts` 的链路只有 bodyParser → coalesceBody → verifyCSRFToken → router),handler 内部必须**显式使用 `sequelize.transaction()`**。同时注意 **`User.rotateJwtSecret` 的真实签名是 `(options: SaveOptions)`**(内部 `this.save(options)`,`server/models/User.ts:598`),不是无参调用——必须传 `{ transaction }`。参照 `auth.delete` 端点的做法(`server/routes/api/auth/auth.ts:188`:该端点通过 `transaction()` 中间件获取事务,再 `user.rotateJwtSecret({ transaction })`)。update handler 骨架:
  ...
  try {
    ...
    // ── reset 路径统一错误处理:所有 token 失败转为 redirect ──
    if (isResetTokenPath) {
      if (
  ...
  >
  > **reset 路径错误处理(v3.15)**:事务内所有 token 相关失败(jti 已消费、Redis value 不匹配、锁内重新验签失败)使用自定义 `ResetTokenConsumedError`,事务外 catch 块统一转为 `ctx.redirect(...?notice=expired-token)`——**不暴露 500**。`jwt.JsonWebTokenError` / `jwt.TokenExpiredError`(锁内验签失败)同样被 catch。登录态路径的错误(如 `AuthenticationError`)不走此 catch,正常 throw,由 Koa 错误处理转为 JSON HTTP error(401)。

  > 登录态路径的 cookie 重签与 JSON 响应都在事务提交**后**进行(签 cookie 是 HTTP 响应操作,不是 DB 写入;不调用 `signIn`——`signIn` 会 `updateSignedIn`、写 `users.signin` 事件并 redirect,这里只需要 cookie)。reset 路径只做 302 redirect,无需 cookie 操作。cookie 参数**不传 `domain`**,复刻现有 self-host `signIn`(`server/utils/authentication.ts:137`)。
...
- **改密成功后的会话处理(保守路径)**:事务内完成 `setPassword()` + `rotateJwtSecret({ transaction })` 后:
  ...
  - **登录态路径**:`rotateJwtSecret()` 会使当前会话同样失效——事务提交后重签 cookie:`user.getSessionToken(expires, "password")` → 设置 `accessToken` cookie(sameSite lax,过期时间同现有 `signIn`)→ 返回 JSON `{ success: true }`。cookie 参数**完全复刻现有 self-host `signIn` 的写法**(`server/utils/authentication.ts:137`——只传 `sameSite: "lax"` + `expires`,**不传 `domain`**)。**不调用 `signIn`**(`signIn` 会 `updateSignedIn`、写 `users.signin` 事件并 redirect,`server/utils/authentication.ts:70-137`,改密不应产生这些副作用);不产生 `users.signin` 审计事件。**认证边界**:`currentPassword` 路径只接受 cookie transport(`parseAuthentication(ctx).transport === "cookie"`,§5.3),header/body/query transport 一律 401 JSON。**失败响应(登录态路径)**:未登录 → 401 JSON;非 cookie transport → 401 JSON;当前密码错误 → 401 JSON(`{ message: "Current password is incorrect" }`,无 cookie 重签、无密码变更);所有失败均返回 JSON HTTP error,**不走 302/HTML**——设置页经 `ApiClient` RPC 调用,`ApiClient` 对非 2xx 按 status code 映射 error 类(`app/utils/ApiClient.ts:186-278`),302/HTML 会导致解析失败。
...
- **场景组件**:`app/scenes/Login/ResetPassword.tsx`。**视觉体系**:不自建布局,照搬同目录 `OAuthAuthorize.tsx` 的组合方式——复用 Login 体系现有组件:`Background`(`./components/Background`)+ `Centered`(`./components/Centered`)包裹,标题用 `Heading`、正文 `Text`、按钮 `ButtonLarge`、输入框 `InputLarge`,加 `PageTitle` 与 `ChangeLanguage`(`detectLanguage()`),与登录页观感一致。行为:
  ...
  - 失败(token 过期/已消费/不匹配/锁内验签失败):服务端在事务外 catch 块中**统一 redirect** 回登录页 `?notice=expired-token`(§5.3 `ResetTokenConsumedError` / `jwt.JsonWebTokenError` / `jwt.TokenExpiredError` 均被 catch),**不暴露 500**——复用 `Notices.tsx` 已有的 expired-token 文案(「该链接已失效,请重新发起」语义吻合),不必新增 case;用户从登录页的"忘记密码"重新发起。
...
- [ ] **登录态路径所有失败返回 JSON HTTP error**:未登录 → 401;非 cookie transport → 401;当前密码错误 → 401;**不走 302/HTML**(`ApiClient` 对 302/HTML 解析失败)。
- [ ] 改密成功调用 `rotateJwtSecret({ transaction })` 吊销全部旧会话(含未消费的 reset token);登录态路径事务提交后当场重签(cookie 参数复刻现有 `signIn`,不传 `domain`)。
...
21. **(v3.13/v3.14)设置页改密 RPC 响应形态**:登录态路径 POST `/auth/password/update`(带 `currentPassword` + `password`,RPC 经 `ApiClient`)成功时返回 JSON `{ success: true }` + 重签 cookie(**cookie 不含 `domain`,复刻现有 `signIn`**),**不返回 302/HTML**;前端无跳转,在设置页原地提示成功;旧 cookie 失效,新 cookie 可用;**不产生 `users.signin` 审计事件**(改密不是登录);reset token native form 路径仍返回 302(§5.3);两条路径的响应形态不能混淆。**失败响应(均为 JSON HTTP error)**:当前密码错误 → 401 JSON(无 cookie 重签、无密码变更);非 cookie transport → 401 JSON;未登录 → 401 JSON。
22. **(v3.14)reset token 并发双提交**:同一 reset token 同时发起两次 POST `/auth/password/update`,只有一次成功改密,另一次因 GETDEL 返回 null 被拒绝(redirect `?notice=expired-token`,**不是 500**);不允许两次都成功;此用例用于钉死单次 GETDEL 权威判定——若实现者误用先 GET 后 GETDEL 的两阶段方案,并发请求可能双双通过 GET。
23. **(v3.14/v3.15)登录态改密认证边界(transport 校验)**:
    ...
    g. cookie transport + **错误** `currentPassword` → 401 JSON(无 cookie 重签、无密码变更、无事件)。
