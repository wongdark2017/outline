# Outline 邮箱 + 密码登录功能 — 技术方案 v3.16

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.16(十八轮代码评审后)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.15.md`。本稿为独立全文,以本稿为准。
...
| # | 评审意见 | 处理 |
|---|---|---|
| 1(High) | 登录态 cookie 重签使用了事务外的旧 `user` 实例:事务内 `lockedUser.rotateJwtSecret()` 改了 `lockedUser.jwtSecret`,但事务后用 `user.getSessionToken(...)` 签 cookie——`user` 仍持有旧 `jwtSecret`,签出的 token 在事务提交后立即无效 | §5.3 骨架改为:事务内用 `lockedUser.getSessionToken()` 生成新 token 并作为事务返回值,事务后用返回的 token 设置 cookie;相关叙述同步 |
| 2(Medium) | `ApiClient` 对 **任何 401** 触发 `stores.auth.logout()`(`app/utils/ApiClient.ts:186`),登录态改密输错 `currentPassword` 返回 401 → 前端自动登出——用户只是输错了密码,不应丢掉会话 | §5.3 登录态 `currentPassword` 校验失败改为 `ValidationError`(400),而非 `AuthenticationError`(401);保留 401 仅用于"未登录/非 cookie transport"等真正的认证失败;§7.3 / §8 / §9 测试 21 / 23g 同步 |
| 3(Medium) | GETDEL 放在 `setPassword` / `rotateJwtSecret` **之后**,但 §9 测试 19b 描述"模拟 `rotateJwtSecret` 抛错导致回滚,密码未改;同一 reset token 再次使用被拒(jti 已被 GETDEL 消耗)"——`rotateJwtSecret` 在 GETDEL 之前抛错时 GETDEL 根本未执行,token 未消耗,测试 19b 的场景不成立 | §5.3 骨架将 GETDEL 移到 DB 变更(`setPassword` / `save` / `rotateJwtSecret`)之前:锁内验签通过 → GETDEL 消费 token → 执行 DB 变更;若后续 DB 步骤抛错,事务回滚但 token 已消耗(安全方向);测试 19b 场景成立;叙述同步 |
| 4(Medium) | reset token 事务前预验证(decode/type/createdAt/team/user/首次验签,§5.3 步骤 1-4)在 `try/catch` 外,若 `JWT.verify` 失败(签名无效/token 过期)异常不被 catch,变 500 | §5.3 骨架将 `try` 块扩展到包裹整个 reset 验证链;catch 块同时捕获事务前和事务内的 token 失败,统一 redirect `?notice=expired-token` |
| 5(Low) | 测试 22 将并发双提交的第二个请求失败绑定到"GETDEL 返回 null",但有 `SELECT ... FOR UPDATE` 后第二个请求更可能在锁内重新验签阶段失败(因 `jwtSecret` 已被第一个事务轮转) | §9 测试 22 放宽断言:"一个成功,一个 redirect `?notice=expired-token`,不允许两次都成功,不允许 500"——不绑定具体失败点(GETDEL / 验签 / 行级锁阻塞均可) |
| 6(Low) | 骨架 `JWT.verify(...)` 用大写 `JWT`(Outline 封装),catch 用小写 `jwt.JsonWebTokenError`(原生 `jsonwebtoken` 包)——两个不同的 import 用了不同大小写,容易误导实现者以为是同一个包 | §5.3 骨架统一 import 命名:Outline 封装用 `JWT`(`@server/utils/jwt`),原生错误类用 `{ JsonWebTokenError, TokenExpiredError }` 具名导入(`jsonwebtoken` 包),catch 块改为 `err instanceof JsonWebTokenError` |
...
- **事务要求(v3.10)**:update handler 的落库操作涉及多步写入(`setPassword` → `save` → `rotateJwtSecret` → 写事件),必须在同一事务内完成——半截失败(如 `rotateJwtSecret` 成功但事件写入失败)会导致状态不一致。`/auth` app **不像 `/api` 有全局 `transaction()` 中间件**(`server/routes/auth/index.ts` 的链路只有 bodyParser → coalesceBody → verifyCSRFToken → router),handler 内部必须**显式使用 `sequelize.transaction()`**。同时注意 **`User.rotateJwtSecret` 的真实签名是 `(options: SaveOptions)`**(内部 `this.save(options)`,`server/models/User.ts:598`),不是无参调用——必须传 `{ transaction }`。参照 `auth.delete` 端点的做法(`server/routes/api/auth/auth.ts:188`:该端点通过 `transaction()` 中间件获取事务,再 `user.rotateJwtSecret({ transaction })`)。update handler 骨架:
  ...
  import JWT from "@server/utils/jwt";                          // Outline 封装
  import { JsonWebTokenError, TokenExpiredError } from "jsonwebtoken"; // 原生错误类
  ...
  try {
    // ── reset token 路径:事务前预验证(fix v3.16:包裹在 try/catch 内) ──
    // 步骤 1-4(decode/type/createdAt/team/user/首次验签)在事务外执行,
    // 但在 try 块内——失败(过期/签名无效/type 不匹配/用户不存在)
    // 由下方 catch 统一转为 redirect,不暴露 500。
    // (步骤 5 jti 消费在事务内执行,见下)
    ...
    // ── reset 路径统一错误处理(fix v3.16:覆盖事务前预验证 + 事务内失败) ──
    // try 块包裹了 reset token 的全部验证链:事务前的 decode/type/team/
    // user/首次验签(步骤 1-4)+ 事务内的锁内重新验签/GETDEL,
    // 任何 token 相关失败都在此统一转为 redirect,不暴露 500。
    if (isResetTokenPath) {
      if (
  ...
  >
  > **reset 路径错误处理(v3.15 / v3.16 扩展)**:`try` 块包裹了 reset token 的**全部验证链**——事务前的预验证(decode / type / createdAt / team / user / 首次验签,步骤 1-4)和事务内的锁内重新验签 / GETDEL;任何 token 相关失败(jti 已消费、Redis value 不匹配、锁内验签失败、**事务前验签失败**)使用自定义 `ResetTokenConsumedError` 或原生 `JsonWebTokenError` / `TokenExpiredError`,事务外 catch 块统一转为 `ctx.redirect(...?notice=expired-token)`——**不暴露 500**。登录态路径的错误(如 `AuthenticationError` / `ValidationError`)不走此 catch,正常 throw,由 Koa 错误处理转为 JSON HTTP error。
  >
  ...
  >
  > **JWT import 命名约定(v3.16)**:Outline 封装的 JWT 工具(`@server/utils/jwt`)导入为大写 `JWT`(用于 `JWT.verify` / `JWT.sign`);原生 `jsonwebtoken` 包的错误类用具名导入 `{ JsonWebTokenError, TokenExpiredError }`——两者来自不同包,不同命名风格避免混淆。
...
- **场景组件**:`app/scenes/Login/ResetPassword.tsx`。**视觉体系**:不自建布局,照搬同目录 `OAuthAuthorize.tsx` 的组合方式——复用 Login 体系现有组件:`Background`(`./components/Background`)+ `Centered`(`./components/Centered`)包裹,标题用 `Heading`、正文 `Text`、按钮 `ButtonLarge`、输入框 `InputLarge`,加 `PageTitle` 与 `ChangeLanguage`(`detectLanguage()`),与登录页观感一致。行为:
  ...
  - 失败(token 过期/已消费/不匹配/锁内验签失败/事务前验签失败):服务端在 catch 块中**统一 redirect** 回登录页 `?notice=expired-token`(§5.3 `ResetTokenConsumedError` / `JsonWebTokenError` / `TokenExpiredError` 均被 catch,try 块覆盖事务前预验证 + 事务内失败),**不暴露 500**——复用 `Notices.tsx` 已有的 expired-token 文案(「该链接已失效,请重新发起」语义吻合),不必新增 case;用户从登录页的"忘记密码"重新发起。
...
- [ ] **reset 路径所有 token 失败(已消费、Redis 不匹配、锁内验签失败)统一 redirect `?notice=expired-token`**:使用自定义 `ResetTokenConsumedError`,事务外 catch 块统一转 redirect,**不暴露 500**;登录态路径错误正常 throw 为 JSON HTTP error。
...
- [ ] 改密成功调用 `rotateJwtSecret({ transaction })` 吊销全部旧会话(含未消费的 reset token);登录态路径事务**内**用 `lockedUser.getSessionToken()` 生成新 token(**不能用事务外的 `user` 实例**——`rotateJwtSecret` 只修改了 `lockedUser.jwtSecret`,旧实例签的 token 立即无效),事务提交后用返回值设置 cookie(cookie 参数复刻现有 `signIn`,不传 `domain`)。
...
- [ ] **reset token 全部验证链(事务前预验证 + 事务内锁内验证)包裹在同一 `try/catch` 内**:事务前步骤 1-4(decode/type/team/user/首次验签)失败不暴露 500,统一由 catch 转为 redirect `?notice=expired-token`。
- [ ] **JWT import 命名统一**:Outline 封装用 `JWT`(`@server/utils/jwt`),原生错误类用具名导入 `{ JsonWebTokenError, TokenExpiredError }`(`jsonwebtoken` 包),不混用大小写。
...
23. **(v3.14/v3.15)登录态改密认证边界(transport 校验)**:
    ...
    g. cookie transport + **错误** `currentPassword` → **400 JSON**(`ValidationError`,**不是 401**——401 会触发 `ApiClient` 的 `stores.auth.logout()`,设置页输错密码不应导致前端登出;无 cookie 重签、无密码变更、无事件)。
24. **(v3.15)同一用户多个不同 reset token 并发**:为同一用户生成两个不同的 reset token(两次调用 `getPasswordResetToken`,两个不同的 jti 均写入 Redis),同时发起两次 POST `/auth/password/update`(各自携带不同 token + 不同新密码)。断言:只有一个成功改密,另一个被拒(锁内重新验签失败,因 `jwtSecret` 已被第一个事务轮转);最终密码是第一个成功事务设置的那个;不允许两次都成功——此用例钉死 `SELECT ... FOR UPDATE` + 锁内重新验签;若实现者省略行级锁,两个事务各自用旧 `jwtSecret` 验签通过、各自 GETDEL 成功、最后一次覆盖前一次密码。
