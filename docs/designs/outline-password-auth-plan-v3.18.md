# Outline 邮箱 + 密码登录功能 — 技术方案 v3.18

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.18(二十轮代码评审后)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.17.md`。本稿为独立全文,以本稿为准。
...
- **事务要求(v3.10)**:update handler 的落库操作涉及多步写入(`setPassword` → `save` → `rotateJwtSecret` → 写事件),必须在同一事务内完成——半截失败(如 `rotateJwtSecret` 成功但事件写入失败)会导致状态不一致。`/auth` app **不像 `/api` 有全局 `transaction()` 中间件**(`server/routes/auth/index.ts` 的链路只有 bodyParser → coalesceBody → verifyCSRFToken → router),handler 内部必须**显式使用 `sequelize.transaction()`**。同时注意 **`User.rotateJwtSecret` 的真实签名是 `(options: SaveOptions)`**(内部 `this.save(options)`,`server/models/User.ts:598`),不是无参调用——必须传 `{ transaction }`。参照 `auth.delete` 端点的做法(`server/routes/api/auth/auth.ts:188`:该端点通过 `transaction()` 中间件获取事务,再 `user.rotateJwtSecret({ transaction })`)。update handler 骨架:
  ...
  // ── reset payload 结构化校验(fix v3.18) ──
  // JWT.JwtPayload 的自定义字段本质上是 [key: string]: any(@types/jsonwebtoken
  // index.d.ts:123),decode 后无类型保证;不做 shape validation 则后续
  // .id / .teamId 等访问依赖 as 断言,运行时可能是 undefined / number。
  ...
  function isPasswordResetPayload(p: unknown): p is PasswordResetPayload {
    if (typeof p !== "object" || p === null) { return false; }
    const o = p as Record<string, unknown>;
    return (
      typeof o.id === "string" &&
      typeof o.teamId === "string" &&
      o.type === "password-reset" &&
  ...
  if (currentPassword !== undefined) {
    ...
    try {
      ...
      try {
        rawPayload = getJWTPayload(resetToken);
      } catch {
        throw new ResetTokenConsumedError("Invalid reset token");
      }

      // 结构化校验(fix v3.18):确认 id / teamId / type / createdAt / jti
      // 全部存在且为 string;否则 decode 出的 payload 不可信。
      if (!isPasswordResetPayload(rawPayload)) {
        throw new ResetTokenConsumedError("Invalid reset token payload");
      }
      // rawPayload 类型已收窄为 PasswordResetPayload——
      // 后续 .id / .teamId / .createdAt / .jti 均为 string,无需 as 断言。

      // createdAt 校验(fix v3.18):防御 Invalid Date 和未来时间。
      // new Date("abc").getTime() 是 NaN,Number.isFinite(NaN) 为 false;
      // 拒绝未来时间,避免时钟异常或篡改将 token 有效期拉长。
      const createdAt = new Date(rawPayload.createdAt);
      if (
      ...
      if (rawPayload.teamId !== team.id) {
        throw new ResetTokenConsumedError("Team mismatch");
      }
      const resetUser = await User.findOne({
        where: { id: rawPayload.id, teamId: team.id },
      });
      if (!resetUser) {
      ...
      try {
        JWT.verify(resetToken, resetUser.jwtSecret);
      } catch {
      ...
      updateContext = {
        ...
        user: resetUser,
        payload: rawPayload,
        jtiKey: `password-reset:jti:${rawPayload.jti}`,
        resetToken,
      };
    } catch (err) {
  ...
  try {
    const newAccessToken = await sequelize.transaction(async (transaction) => {
      ...
      // hooks: false 是有意为之:此处只改了 passwordHash,不走 changeset/通知 hooks。
      // ⚠️ 仅允许用于只保存 passwordHash 的这一次 save。
    ...
    // ── reset 路径:事务内错误统一 redirect(fix v3.16 / v3.17) ──
    ...
    // 登录态路径错误(ValidationError / AuthenticationError)不走此 catch,正常 throw。
    if (updateContext.kind === "reset") {
      if (
  ...
  >
  > **reset 路径两区错误处理(v3.15 / v3.16 / v3.17 / v3.18 修正)**:错误处理分为两个 try/catch 区域。**区域一(预验证)**:构建 `ResetContext` 的 try/catch,捕获 `ResetTokenConsumedError` 后立即 redirect `?notice=expired-token` 并 return——预验证失败不进入事务。所有预验证失败仍归一化为 `ResetTokenConsumedError`(v3.17):关键原因是 `getJWTPayload`(`server/utils/jwt.ts:8`)抛出 `AuthenticationError`——`httpErrors(401, ...)` 工厂返回值,**不是 class**,`instanceof` 不匹配;payload shape 校验失败、createdAt 无效/未来/过期、team 不匹配、user 不存在、首次验签失败均归一化。**区域二(事务)**:包裹 `sequelize.transaction()` 的 try/catch,捕获事务内的 `ResetTokenConsumedError`(GETDEL 返回 null / Redis 坏值)、`JsonWebTokenError`(锁内重新验签失败)、`TokenExpiredError`,统一 redirect——**不暴露 500**。登录态路径的错误(`ValidationError` / `AuthenticationError`)在**两个 try/catch 之前**抛出(构建 `LoginContext` 时),不被任一 catch 捕获,正常 throw 为 JSON HTTP error。
  >
  > **Discriminated union 与类型收窄(v3.18)**:鉴权结果归一到 `PasswordUpdateContext = ResetContext | LoginContext`,以 `kind` 字段区分。事务闭包内 `updateContext.kind === "reset"` 时编译器保证 `.payload`(已收窄为 `PasswordResetPayload`)、`.jtiKey`、`.resetToken` 存在且类型正确;`kind === "login"` 时保证 `.currentPassword`、`.expires` 存在。取代了 v3.17 的 `let payload: JWT.JwtPayload | undefined` + `let jtiKey: string | undefined` + `isResetTokenPath` 布尔推断模式——后者在闭包捕获场景下 TypeScript 无法收窄,实现者被迫加 `!` 非空断言或 `as` 类型断言,丢失编译期安全性。`updateContext.user.id` 在两条路径上均来自已校验的 `User` 实例,`findByPk` 不再需要断言。
  >
  > **Reset payload 结构化校验(v3.18)**:`getJWTPayload` 返回 `JWT.JwtPayload`,其自定义字段本质上是 `[key: string]: any`(`@types/jsonwebtoken/index.d.ts:123`)——v3.17 用 `payload.createdAt as string`、`payload.id as string` 绕过类型检查,运行时若 payload 被篡改或签名算法降级导致字段缺失/类型错误,`as` 断言不会拦截。v3.18 在 decode 后立即调用 `isPasswordResetPayload()` 类型守卫,校验 `id` / `teamId` / `type` / `createdAt` / `jti` 全部存在且为 `string`,失败归一化为 `ResetTokenConsumedError`;通过后 `rawPayload` 类型收窄为 `PasswordResetPayload`,后续所有访问无需 `as` 断言。
  >
  > **createdAt 校验加固(v3.18)**:`new Date(undefined)` 和 `new Date("abc")` 均产生 Invalid Date,其 `getTime()` 返回 `NaN`;`NaN < subMinutes(...)` 为 `false`,等于没有判过期——任何非法 `createdAt` 值都能绕过时间窗口检查。v3.18 先 `Number.isFinite(createdAt.getTime())` 拒绝 Invalid Date,再 `createdAt > new Date()` 拒绝未来时间(防止时钟异常或 payload 篡改将有效期拉长),最后判 15 分钟过期。
  >
  ...
  >
  > **JWT import 约定(v3.16 / v3.17 / v3.18 修正)**:`server/utils/jwt.ts` **没有 default export**(`server/utils/jwt.ts:8`),只导出 `getJWTPayload` / `getUserForJWT` 等具名函数;其内部 `import JWT from "jsonwebtoken"`(`:2`)但不重新导出。因此 password 插件的 import 分三组:`import JWT, { JsonWebTokenError, TokenExpiredError } from "jsonwebtoken"` + `import { getJWTPayload } from "@server/utils/jwt"` + `import { AuthenticationError, ValidationError, InternalError } from "@server/errors"`(v3.18:骨架中使用的全部 error helper 显式列出,消除照抄时的编译缺口)。
...
- [ ] **reset 路径所有 token 失败统一 redirect `?notice=expired-token`(两区 catch)**:预验证失败(decode / payload shape / createdAt / team / user / 首次验签)归一化为 `ResetTokenConsumedError`,在**区域一 catch** redirect;事务内失败(锁内验签 `JsonWebTokenError` / GETDEL `ResetTokenConsumedError` / Redis 坏值 `ResetTokenConsumedError`)在**区域二 catch** redirect——**不暴露 500**;登录态路径错误在两个 catch 之前抛出,正常 throw 为 JSON HTTP error。
...
- [ ] **reset token 验证链分两区 catch(fix v3.18)**:预验证(构建 `ResetContext`)有独立 try/catch,失败 redirect 后 return,不进入事务;事务(锁内验签 + GETDEL + Redis 解析)有独立 try/catch,失败 redirect。两区都只捕获 `ResetTokenConsumedError` / `JsonWebTokenError` / `TokenExpiredError`,其他错误 re-throw。登录态路径错误在两区**之前**抛出,不被捕获。
- [ ] **JWT import 正确**:`server/utils/jwt.ts` 无 default export;`JWT` / `JsonWebTokenError` / `TokenExpiredError` 全部从 `jsonwebtoken` 导入;`getJWTPayload` 从 `@server/utils/jwt` 具名导入;**`AuthenticationError` / `ValidationError` / `InternalError` 从 `@server/errors` 导入**(fix v3.18:骨架中使用的全部 error helper 显式列出)——与 `User.ts:3` 和 `server/utils/jwt.ts:2` 的写法一致。
- [ ] **reset payload 结构化校验(fix v3.18)**:`getJWTPayload` 返回 `JWT.JwtPayload`(自定义字段为 `any`),decode 后必须经 `isPasswordResetPayload()` 类型守卫校验 `id` / `teamId` / `type` / `createdAt` / `jti` 全部存在且为 `string`;失败归一化为 `ResetTokenConsumedError`;通过后类型收窄为 `PasswordResetPayload`,后续访问不需要 `as` 断言。
- [ ] **createdAt 校验防御 Invalid Date 和未来时间(fix v3.18)**:`new Date(payload.createdAt)` 后先 `Number.isFinite(getTime())` 拒绝 Invalid Date(否则 `NaN < subMinutes(...)` 为 `false`,等于没有判过期),再 `createdAt > new Date()` 拒绝未来时间(防时钟异常 / payload 篡改拉长有效期),最后判 15 分钟过期。
...
- [ ] **鉴权结果使用 discriminated union(fix v3.18)**:`PasswordUpdateContext = ResetContext | LoginContext`,以 `kind` 字段区分;事务闭包内用 `updateContext.kind === "reset"` 收窄,编译器保证字段存在;不再使用 `isResetTokenPath` 布尔推断外部 `let` 变量已赋值(闭包捕获场景下 TypeScript 无法收窄)。
...
7. 三键限流任一超限返回 429;
8. reset token(字段名 `resetToken`):正常消费一次成功且**改的是 token `id` 声明的那个用户**,成功后 redirect `?notice=password-updated` **且响应不携带会话 cookie**;二次使用被拒(jti 已消费);过期被拒;`teamId` 不匹配当前请求解析出的 team 被拒;Redis value 与 payload 不一致被拒;**Redis value 非 JSON(如手动写入坏值)或字段缺失(如 `{ userId: 123 }` 非 string)→ redirect `?notice=expired-token`,不 500**(fix v3.18:钉死 Redis parse + shape 校验);**payload 缺少必需字段(如无 `jti` / `createdAt`)→ redirect,不 500**(fix v3.18:钉死 `isPasswordResetPayload` shape 校验);**`createdAt` 为非法日期字符串(如 `"abc"`)或未来时间 → redirect,不 500**(fix v3.18:钉死 Invalid Date / 未来时间校验);update 请求体附带的 email/userId 参数被拒(strict schema 400);失败时 redirect 到登录页 `?notice=expired-token`;
