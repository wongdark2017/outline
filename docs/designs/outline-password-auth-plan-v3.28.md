# Outline 邮箱 + 密码登录功能 — 技术方案 v3.28

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.28(三十轮代码评审后)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.27.md`。本稿为独立全文,以本稿为准。
...
2. **team 解析前 IP-only 预限流(fix v3.27 / v3.28)**:正式三键限流需要 `teamId`,但 team 解析失败 / password provider 不可用的请求也不能完全绕过 password 端点限流。先 consume 一个轻量 IP-only key(如 `password-login-preteam:ip:${ip}`),只用于覆盖未知 hostname/custom domain/provider unavailable 的探测与日志放大;阈值应宽于正式登录 IP key,避免正常多 workspace 登录被过早挡住。**阈值建议为正式登录 IP key 的 3–5 倍,窗口与正式 IP key 相同**(fix v3.28);具体值可按部署规模调整,但必须满足 §9 测试 4 的跨 workspace 不误伤要求。解析 team 成功后仍继续执行第 4 步正式三键限流;预限流不是正式爆破防线的替代。**Redis 故障降级(fix v3.28)**:pre-IP-limit 的 Redis/limiter 调用外包 try/catch;抛错时降级为跳过预限流、继续后续 team 解析与正式三键限流——成本防护不改变用户可见错误形态,与 §5.3 EXISTS 降级同口径。
...
- **team 解析前 IP-only 预限流(fix v3.27 / v3.28)**:与登录端点同理,reset 的正式 key 需要 `teamId`,但未知 hostname/custom domain/provider unavailable 请求也不能无限制打到解析与错误日志。先 consume 一个轻量 IP-only key(如 `password-reset-preteam:ip:${ip}`),**阈值建议为正式 reset IP key 的 3–5 倍,窗口与正式 reset IP key 相同**(fix v3.28);具体值可按部署规模调整,但必须满足 §9 测试 4 的跨 workspace 不误伤要求。team 解析成功后仍继续执行正式三键限流。**Redis 故障降级(fix v3.28)**:pre-IP-limit 的 Redis/limiter 调用外包 try/catch;抛错时降级为跳过预限流、继续后续 team 解析与正式三键限流——成本防护不改变用户可见错误形态,与 §5.3 EXISTS 降级同口径。
...
- [ ] **team 解析前 IP-only 预限流(fix v3.27 / v3.28)**:登录与 reset 都先 consume 轻量 IP-only key(`password-login-preteam:ip` / `password-reset-preteam:ip`),覆盖 team 不存在、custom domain 探测、provider unavailable 等没有 `teamId` 的失败路径;**建议阈值为对应正式 IP key 的 3–5 倍,窗口相同**。具体值可按部署规模调整,但必须满足 §9 测试 4 的跨 workspace 不误伤要求。team 解析成功后仍必须执行正式三键限流,预限流不得替代正式爆破/邮件轰炸防线。pre-IP-limit Redis/limiter 调用必须 fail-open:外包 try/catch,抛错时跳过预限流并继续 team 解析与正式三键限流,不因成本防护短暂故障改变用户可见错误形态。
...
4. **同邮箱多 workspace 隔离(fix v3.26 / v3.28:含限流隔离)**:两个 team 各有同邮箱用户、密码不同,从 A 入口用 B 的密码登录被拒,从各自入口用各自密码登录成功且会话归属正确;reset 流程同样只命中当前 team 的用户;**限流互不污染**:对 A workspace 的同邮箱连续发起接近阈值的错误登录/reset,B workspace 同邮箱的正常登录/reset 不受限流影响(限流 key 含 `teamId`,A 的 `teamId:emailHmac` 消耗不影响 B 的同名 key);team 解析前 IP-only 预限流按正式 IP key 的 3–5 倍、同窗口配置,不得在此正常跨 workspace 场景中先于正式 key 误伤。
...
7. **登录端点限流(fix v3.27:具体 key 与执行顺序)**:
    ...
    c. **team 解析前 IP-only 预限流(fix v3.28:阈值与降级)**:未知 hostname/custom domain 或 password provider unavailable 连续请求超过预限流阈值 → 429;该场景没有 `teamId`,不得写入 `teamId:emailHmac` key;预限流阈值按正式登录 IP key 的 3–5 倍、同窗口配置;正常解析成功的请求仍继续走正式三键限流。stub pre-IP limiter Redis 调用抛错时,请求不 500,而是跳过预限流并继续 team 解析与正式三键限流。
...
25. **(v3.23 / v3.26 / v3.27:key 带 team 维度 + 预限流)reset 端点限流**:
    ...
    g. **(fix v3.27 / v3.28)team 解析前 IP-only 预限流**:未知 hostname/custom domain 或 password provider unavailable 连续请求超过预限流阈值 → 429;该场景没有 `teamId`,不得写入 `teamId:emailHmac` key;预限流阈值按正式 reset IP key 的 3–5 倍、同窗口配置;team 解析成功后仍会继续执行正式三键限流;stub pre-IP limiter Redis 调用抛错时,请求不 500,而是跳过预限流并继续 team 解析与正式三键限流;
