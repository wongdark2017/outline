# Outline 邮箱 + 密码登录功能 — 技术方案 v3.5

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.5(七轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md`、`-v3.3.md`、`-v3.4.md`。本稿为独立全文,以本稿为准。
...
| # | 评审意见 | 处理 |
|---|---|---|
| 1 | `client.post("/auth/password/reset", …)` 写法错误:`ApiClient` 默认 `baseUrl` 是 `/api`(`app/utils/ApiClient.ts:55`,拼接见 `:112`),会请求到 `/api/auth/password/reset` | §7.1 修正为 **`client.post("/password/reset", { email }, { baseUrl: "/auth" })`**,与 passkeys 调 `/auth` 下端点的现有先例一致(`AuthenticationProvider.tsx:81`) |
| 2 | reset token 改密成功后"当场 `signIn` 自动登录"未说明 tradeoff:刚用完的重置链接自动创建会话,安全上偏激进 | §5.3 / §7.2 改为**保守路径**:`rotateJwtSecret()` 后 redirect 登录页 `?notice=password-updated`,要求用新密码登录;`Notices.tsx` 增加该成功提示 case;自动登录作为显式标注 tradeoff 的备选记入 §10 |
| 3 | `ResetPassword.tsx` 未明确复用 Login 视觉体系,易做出割裂 UI(非阻塞) | §7.2 明确照搬 `OAuthAuthorize.tsx` 的组合方式:复用 `Background` / `Centered` / `Heading` / `ButtonLarge` / `PageTitle` / `ChangeLanguage` 等 Login 目录现有组件 |

**v3.3 → v3.4(上轮,已收口)**:3 个密码字段全部 `@SkipChangeset`(`Model.insertEvent` 会把 `previousChangeset` 写入 `events.changes`,hash 不允许落 events 表),审计改用显式事件 `data: { passwordChanged: true }`;补密码找回前端闭环——登录页"忘记密码"子状态 + 新增公开路由 `/reset-password` 与 `app/scenes/Login/ResetPassword.tsx`。
...
- 经现有 mailer 队列发送 `PasswordResetEmail`,**链接指向 `${team.url}/reset-password?token=…`**(§7.2 的公开页面;域名用 `team.url` 拼,不用根域)。**注意**:模板文件在插件目录内,要走 `.schedule()` 队列链路就必须在 `plugins/password/server/index.ts` 注册 **`Hook.EmailTemplate`**(参照 `plugins/passkeys/server/index.ts`)——`EmailTask` 是按模板名从注册表(`server/emails/templates/index.ts`)取类的,不注册则队列侧找不到模板。
...
- 密码策略:最少 12 位。
- **改密成功后的会话处理(v3.5 修正,取保守路径)**:落库 `setPassword()` 后调用 **`user.rotateJwtSecret()`** 使该用户全部旧 JWT 失效(含其余未消费的 reset token,见 §5.2),然后:
...
- **场景组件**:`app/scenes/Login/ResetPassword.tsx`。**视觉体系(v3.5 明确)**:不自建布局,照搬同目录 `OAuthAuthorize.tsx` 的组合方式——复用 Login 体系现有组件:`Background`(`./components/Background`)+ `Centered`(`./components/Centered`)包裹,标题用 `Heading`、正文 `Text`、按钮 `ButtonLarge`、输入框 `InputLarge`、加 `PageTitle` 与 `ChangeLanguage`(`detectLanguage()`),与登录页观感一致。行为:
  - 从 query string 读 `token`;无 token 时直接展示"链接无效"并引导回登录页;
  - 表单:新密码 + 确认新密码(前端校验一致性与最少 12 位,与 §5.3 服务端策略同口径);
  - 提交:**native form POST 到 `/auth/password/update`**,hidden input 携带 `token` 与 CSRF 字段(与 §7.1 登录表单同款注入逻辑;此路径不自动登录,无需 `client` 字段)——服务端验证通过即 `setPassword` + `rotateJwtSecret()`,**redirect 到登录页 `?notice=password-updated`**(§5.3 保守路径),用户用新密码登录;
...
- **Notices 文案**:`app/scenes/Login/components/Notices.tsx` 的 `Message` switch 当前没有 password 相关 case,未注册的 notice 会落到默认的 "unknown error"。需新增三个 case:
  ...
  - `password-updated` → 「密码已更新,请使用新密码登录。」(v3.5 新增,§5.3 保守路径的落点;`Notices` 组件目前统一渲染 warning 图标,该条是成功提示——实现时接受 warning 样式,或给该 case 单独换图标/样式,二选一,不阻塞)
...
- **设置页**:修改密码卡片放**个人 Profile / Preferences**,不放团队 Security;走登录态路径(当前密码 + 新密码,POST `/auth/password/update`,RPC 调用同样注意 `{ baseUrl: "/auth" }`)。
...
- [ ] native form 提交(登录表单、`/reset-password` 表单)携带 CSRF hidden input(`CSRF.fieldName` ← `CSRF.cookieName`),通过 `/auth` app 全局 `verifyCSRFToken()`;"忘记密码"等 RPC 请求经 `ApiClient` 自动带 CSRF header,且必须显式传 `{ baseUrl: "/auth" }`。
...
7. 三键限流任一超限返回 429;
8. reset token:正常消费一次成功且**改的是 token `id` 声明的那个用户**,成功后 redirect `?notice=password-updated` **且响应不携带会话 cookie**;二次使用被拒(jti 已消费);过期被拒;`teamId` 不匹配当前请求解析出的 team 被拒;Redis value 与 payload 不一致被拒;update 请求体附带的 email/userId 参数被忽略或拒绝;失败时 redirect 到登录页 `?notice=expired-token`;
...
- **reset 成功后自动登录(显式 tradeoff,v3.5 移入此处)**:体验更顺(改完密码直接进应用),但意味着"持有邮件链接 → 直接获得会话",放大邮箱被短暂访问的影响面;且 update 端点必须复刻登录路径的 suspended 预检查、`client` hidden input 与 Desktop 分支、限流口径,维护两份一致性。若未来采纳:逐项对齐 §5.1 的检查清单后,把 §5.3 reset 路径的 redirect 换成 `signIn(ctx, "password", …)`。首版不做。
