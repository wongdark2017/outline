# Outline 邮箱 + 密码登录功能 — 技术方案 v3.6

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.6(八轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md`、`-v3.3.md`、`-v3.4.md`、`-v3.5.md`。本稿为独立全文,以本稿为准。
...
| # | 评审意见 | 处理 |
|---|---|---|
| 1(High) | `/reset-password` 表单的 reset token hidden input 不能命名为 `token`:`parseAuthentication` 在无 Authorization header 时会把 body 里的 `token` 字段当认证 token 并返回 `transport: "body"`(`server/middlewares/authentication.ts:105`),`verifyCSRFToken` 见非 cookie transport 直接跳过 CSRF(`server/middlewares/csrf.ts:51`)——已有会话 cookie 的场景下,改名前 CSRF 双提交校验整体失效 | 字段统一改名 **`resetToken`**,§5.3(handler/schema)、§7.2(hidden input)、§8、§9 同步;并新增显式说明:**任何 `/auth/password/*` 请求体中禁止出现名为 `token` 的字段**。同时补充剩余风险说明:改名只恢复"存在 accessToken cookie 时"的 CSRF 保护,匿名 POST(无任何会话 cookie)本就不在现有 CSRF 模型覆盖内——与现有 email 登录表单同一安全姿态,首版接受,route-level 强制 CSRF 列入 §10(见 §7.2 注) |

**v3.4 → v3.5(上轮,已收口)**:前端 RPC 调 `/auth` 下端点必须显式 `{ baseUrl: "/auth" }`(`ApiClient` 默认 `/api`);reset 改密成功后**不自动登录**,redirect `?notice=password-updated` 要求用新密码重新登录(自动登录作为显式 tradeoff 移入 §10);`ResetPassword.tsx` 明确复用 Login 视觉体系(`Background`/`Centered` 等,参照 `OAuthAuthorize.tsx`)。
...
plugins/password/server/auth/schema.ts                ★ zod 校验 schema(update 用 resetToken 字段,§5.3)
...
> **请求体字段命名红线(v3.6)**:**任何 `/auth/password/*` 端点的请求体中禁止出现名为 `token` 的字段**。`parseAuthentication` 在没有 Authorization header 时,会把 body 里的 `token` 字段当作**认证 token** 并返回 `transport: "body"`(`server/middlewares/authentication.ts:105`);而 `verifyCSRFToken` 的 `shouldProtectRequest` 对非 cookie transport **直接跳过 CSRF 校验**(`server/middlewares/csrf.ts:51`)。即:body 里出现 `token` 字段会**整体关闭该请求的 CSRF 双提交校验**(已有 accessToken cookie 的场景下原本应启用),还会让 authMiddleware 误把 reset token 当会话凭据解析。reset token 字段统一命名 **`resetToken`**(§5.3、§7.2),zod schema 亦只接受 `resetToken`。
...
- 鉴权两选一:
  1. **reset token 路径(消费顺序写死)**:请求体字段为 **`resetToken`**(v3.6 改名,理由见 §5.0 红线;zod schema 只接受 `resetToken`,出现 `token` 字段直接 400)。验证参照 `getUserForEmailSigninToken` 的形态(`server/utils/jwt.ts:99`):
...
- **场景组件**:`app/scenes/Login/ResetPassword.tsx`。**视觉体系**:不自建布局,照搬同目录 `OAuthAuthorize.tsx` 的组合方式——复用 Login 体系现有组件:`Background`(`./components/Background`)+ `Centered`(`./components/Centered`)包裹,标题用 `Heading`、正文 `Text`、按钮 `ButtonLarge`、输入框 `InputLarge`,加 `PageTitle` 与 `ChangeLanguage`(`detectLanguage()`),与登录页观感一致。行为:
  ...
  - 提交:**native form POST 到 `/auth/password/update`**,hidden input 携带 **`resetToken`(v3.6 改名,严禁命名为 `token`)** 与 CSRF 字段(与 §7.1 登录表单同款注入逻辑;此路径不自动登录,无需 `client` 字段)。**改名原因(High,§5.0 红线)**:body 里名为 `token` 的字段会被 `parseAuthentication` 当认证 token(`transport: "body"`,`server/middlewares/authentication.ts:105`),`verifyCSRFToken` 对非 cookie transport 直接放行(`server/middlewares/csrf.ts:51`)——即"带会话 cookie 的用户访问重置页"这一场景下 CSRF 双提交校验会被整体绕过,§8 清单与 §9 测试 14 全部落空。服务端验证通过即 `setPassword` + `rotateJwtSecret()`,**redirect 到登录页 `?notice=password-updated`**(§5.3 保守路径),用户用新密码登录;
...
- **设置页**:修改密码卡片放**个人 Profile / Preferences**,不放团队 Security;走登录态路径(当前密码 + 新密码,POST `/auth/password/update`,RPC 调用同样注意 `{ baseUrl: "/auth" }`;字段名同样遵守 §5.0 红线,不出现 `token`)。
...
- [ ] **请求体字段命名红线:`/auth/password/*` 的 body 中禁止出现名为 `token` 的字段**——`parseAuthentication` 会将其当认证 token(`transport: "body"`),导致 `verifyCSRFToken` 整体跳过 CSRF(`server/middlewares/authentication.ts:105`、`server/middlewares/csrf.ts:51`)。reset token 字段固定命名 `resetToken`,zod schema 只接受 `resetToken`。
...
7. 三键限流任一超限返回 429;
8. reset token(字段名 `resetToken`):正常消费一次成功且**改的是 token `id` 声明的那个用户**,成功后 redirect `?notice=password-updated` **且响应不携带会话 cookie**;二次使用被拒(jti 已消费);过期被拒;`teamId` 不匹配当前请求解析出的 team 被拒;Redis value 与 payload 不一致被拒;update 请求体附带的 email/userId 参数被忽略或拒绝;失败时 redirect 到登录页 `?notice=expired-token`;
...
14. **CSRF(v3.6 细化为三条)**:
    a. 携带 accessToken cookie 但缺少 / 错误 CSRF 字段的表单 POST 被 `verifyCSRFToken()` 拒绝;携带正确 hidden input 的提交通过;
    b. **update 请求 body 用 `resetToken` 字段名时,携带 accessToken cookie 的请求仍走 cookie transport、CSRF 校验生效**(对照组:故意把字段命名为 `token` 时 transport 变为 body、CSRF 被跳过——该对照用例用于钉死 §5.0 红线,防止未来改名回退);
    c. update 的 zod schema 拒绝请求体中出现 `token` 字段(400);
...
- **匿名请求的 route-level CSRF(v3.6 移入)**:现有 `verifyCSRFToken` 只覆盖 cookie transport,完全匿名的 POST 不校验(§7.2 说明)。若要对未登录的 reset/update 表单强制 CSRF,需新写 route-level 校验(无视 transport,直接比对 cookie 与 body 字段)或调整全局 middleware 的 `shouldProtectRequest` 策略——后者影响所有 auth 端点,需单独评审。首版沿用现有模型。
...
| 测试(17 项,CSRF 三子项)与安全打磨 | 1.5 – 2.5 天 |
