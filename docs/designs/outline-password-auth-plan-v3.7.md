# Outline 邮箱 + 密码登录功能 — 技术方案 v3.7

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.7(九轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.6.md`。本稿为独立全文,以本稿为准。
...
| # | 评审意见 | 处理 |
|---|---|---|
| 1(Medium) | v3.6 写"zod schema 只接受 `resetToken`,出现 `token` 直接 400",但按现有校验链不会自动成立:`validate()` 只是 `schema.parse(ctx.request)`(`server/middlewares/validate.ts:14`),Zod object **默认剥离未知字段而不报错**,`BaseSchema` 也无全局 strict(`server/routes/api/schema.ts:9`)——只写 `z.object({ resetToken: … })` 时测试 14c 必失败,且带 `token` 的请求仍会先在全局 CSRF 中间件处触发 `transport: "body"` 跳过检查 | §5.0 红线写硬:`/auth/password/*` 的 body schema **必须 `.strict()`** 并显式枚举全部合法字段(含 `_csrf`、`client`——native form 会合法提交它们,漏列会把正常请求也拒掉);同时明确**中间件执行顺序**:`verifyCSRFToken` 先于 route validation 运行,schema 拒绝只是 validation 阶段的纵深,**不能替代字段改名对 CSRF 的修复**(§5.0、§5.3、§8、§9 测试 14 同步) |

**v3.5 → v3.6(上轮,已收口)**:reset token 请求体字段改名 `resetToken`(High——body 里名为 `token` 的字段会被 `parseAuthentication` 当认证 token 返回 `transport: "body"`,`verifyCSRFToken` 对非 cookie transport 直接放行,CSRF 双提交在有会话 cookie 场景整体失效);页面 URL query `?token=` 与 POST body 字段分离;补 CSRF 覆盖范围说明(匿名 POST 本就不在现有模型内,与 email 登录表单同姿态);测试 14 细化三子项含 `token` 命名对照组。
...
plugins/password/server/auth/schema.ts                ★ zod 校验 schema(全部 .strict() + 字段枚举,§5.0)
...
> **请求体字段命名红线**:**任何 `/auth/password/*` 端点的请求体中禁止出现名为 `token` 的字段**。`parseAuthentication` 在没有 Authorization header 时,会把 body 里的 `token` 字段当作**认证 token** 并返回 `transport: "body"`(`server/middlewares/authentication.ts:105`);而 `verifyCSRFToken` 的 `shouldProtectRequest` 对非 cookie transport **直接跳过 CSRF 校验**(`server/middlewares/csrf.ts:51`)。即:body 里出现 `token` 字段会**整体关闭该请求的 CSRF 双提交校验**(已有 accessToken cookie 的场景下原本应启用),还会让 authMiddleware 误把 reset token 当会话凭据解析。reset token 字段统一命名 **`resetToken`**(§5.3、§7.2)。

> **schema 必须 `.strict()`(v3.7,红线的落地方式)**:上一条"出现 `token` 即 400"**不会自动成立**——`validate()` 中间件只是 `schema.parse(ctx.request)`(`server/middlewares/validate.ts:14`),而 Zod object **默认剥离未知字段、不报错**,`BaseSchema` 也没有全局 strict(`server/routes/api/schema.ts:9`)。只写 `z.object({ resetToken: z.string() })` 时,带 `token` 的请求会被**静默剥掉后照常通过校验**。因此 `plugins/password/server/auth/schema.ts` 的三个 body schema **必须显式 `.strict()`,并枚举全部合法字段**。注意两点,否则会引入新问题:
>
> 1. **native form 会合法提交 `_csrf`(`CSRF.fieldName`)与 `client` 字段**(§7.1/§7.2 的 hidden input),`.strict()` 时必须把它们列入 schema,漏列会把正常表单提交也 400 掉。形如:
>
>    ```ts
...
>      body: z
>        .object({
>          resetToken: z.string().optional(),     // reset token 路径
>          currentPassword: z.string().optional(), // 登录态路径
>          password: z.string().min(12),
>          [CSRF.fieldName]: z.string().optional(), // "_csrf",native form 双提交字段
>        })
>        .strict(),
>    });
>    // 登录 schema 同理:email / password / client / _csrf 枚举 + .strict()
>    // reset schema 同理:email / _csrf 枚举 + .strict()
>    ```
>
> 2. **执行顺序决定了 `.strict()` 只是纵深,不是修复**:`/auth` app 的中间件链是 bodyParser → `verifyCSRFToken()` → router(route 内才跑 `validate()`,`server/routes/auth/index.ts:92`)。带 `token` 字段的恶意请求**先**经过 CSRF 中间件(此时已因 `transport: "body"` 被跳过),**后**才被 strict schema 400。所以防 CSRF 绕过的真正修复是**字段改名 `resetToken`**(让合法请求保持 cookie transport);`.strict()` 的作用是在 validation 阶段钉死红线——让任何"把字段叫回 `token`"的回退在测试里立刻爆炸,并顺带拒绝其他垃圾字段。两者缺一不可,不能互相替代。

### 5.1 `POST /auth/password` — 登录

1. `schema.ts` 中的 zod schema 校验 `email` / `password` / `client`(邮箱小写归一;`client` 取值 `Client.Web` / `Client.Desktop`,由表单 hidden input 提交,见 §7;schema 按 §5.0 `.strict()` 并枚举含 `_csrf` 在内的全部字段)。
...
- 经现有 mailer 队列发送 `PasswordResetEmail`,**链接指向 `${team.url}/reset-password?token=…`**(§7.2 的公开页面;域名用 `team.url` 拼,不用根域。页面 URL 的 query 参数名可保留 `token`——它只被前端 SPA 读取,不会进入 `/auth/password/*` 的请求体;进入请求体时必须改名 `resetToken`,见 §5.0 红线)。**注意**:模板文件在插件目录内,要走 `.schedule()` 队列链路就必须在 `plugins/password/server/index.ts` 注册 **`Hook.EmailTemplate`**(参照 `plugins/passkeys/server/index.ts`)——`EmailTask` 是按模板名从注册表(`server/emails/templates/index.ts`)取类的,不注册则队列侧找不到模板。
...
- 在 `AuthenticationProvider.tsx` 新增 `id === "password"` 硬编码分支(与 email / passkeys 同构):邮箱 + 密码输入框,**native form POST 到 `/auth/password`**(参照 passkeys),让 `signIn` 的 302 + Set-Cookie 行为最自然。
...
- **场景组件**:`app/scenes/Login/ResetPassword.tsx`。**视觉体系**:不自建布局,照搬同目录 `OAuthAuthorize.tsx` 的组合方式——复用 Login 体系现有组件:`Background`(`./components/Background`)+ `Centered`(`./components/Centered`)包裹,标题用 `Heading`、正文 `Text`、按钮 `ButtonLarge`、输入框 `InputLarge`,加 `PageTitle` 与 `ChangeLanguage`(`detectLanguage()`),与登录页观感一致。行为:
  - 从 query string 读 `token`(页面 URL 参数,仅前端消费);无 token 时直接展示"链接无效"并引导回登录页;
  - 表单:新密码 + 确认新密码(前端校验一致性与最少 12 位,与 §5.3 服务端策略同口径);
  - 提交:**native form POST 到 `/auth/password/update`**,hidden input 携带 **`resetToken`(严禁命名为 `token`)** 与 CSRF 字段(与 §7.1 登录表单同款注入逻辑;此路径不自动登录,无需 `client` 字段)。**改名原因(High,§5.0 红线)**:body 里名为 `token` 的字段会被 `parseAuthentication` 当认证 token(`transport: "body"`,`server/middlewares/authentication.ts:105`),`verifyCSRFToken` 对非 cookie transport 直接放行(`server/middlewares/csrf.ts:51`)——即"带会话 cookie 的用户访问重置页"这一场景下 CSRF 双提交校验会被整体绕过。服务端验证通过即 `setPassword` + `rotateJwtSecret()`,**redirect 到登录页 `?notice=password-updated`**(§5.3 保守路径),用户用新密码登录;
...
- **设置页**:修改密码卡片放**个人 Profile / Preferences**,不放团队 Security;走登录态路径(当前密码 + 新密码,POST `/auth/password/update`,RPC 调用同样注意 `{ baseUrl: "/auth" }`;字段名遵守 §5.0 红线与 `.strict()` 枚举,不出现 `token`)。
...
- [ ] **请求体字段命名红线:`/auth/password/*` 的 body 中禁止出现名为 `token` 的字段**——`parseAuthentication` 会将其当认证 token(`transport: "body"`),导致 `verifyCSRFToken` 整体跳过 CSRF(`server/middlewares/authentication.ts:105`、`server/middlewares/csrf.ts:51`)。reset token 字段固定命名 `resetToken`。
- [ ] **三个 body schema 全部 `.strict()` 并枚举合法字段(含 `_csrf` / `client`)**:Zod 默认剥离未知字段不报错(`server/middlewares/validate.ts:14`、`server/routes/api/schema.ts:9`),不加 `.strict()` 则"出现 `token` 即 400"不成立;同时牢记 `.strict()` 发生在 route validation 阶段、晚于全局 CSRF 中间件,它是钉死红线的纵深,**不能替代 `resetToken` 改名**(§5.0)。
...
- [ ] native form 提交(登录表单、`/reset-password` 表单)携带 CSRF hidden input(`CSRF.fieldName` ← `CSRF.cookieName`),在 **cookie transport 场景**下通过 `/auth` app 全局 `verifyCSRFToken()`(匿名场景的覆盖范围限制见 §7.2 说明);"忘记密码"等 RPC 请求经 `ApiClient` 自动带 CSRF header,且必须显式传 `{ baseUrl: "/auth" }`。
...
14. **CSRF 与 schema(v3.7 细化为四条)**:
    ...
    c. **strict schema 拒绝未知字段**:请求体中出现 `token` 字段 → 400(`ValidationError`);出现其他任意未声明字段 → 同样 400——注意 Zod 默认剥离未知字段不报错,此用例只有在 schema 带 `.strict()` 时才会通过,用于钉死 §5.0 的 schema 要求;
    d. 三个端点的 schema 各自枚举的合法字段集合与前端实际提交字段一一对应(登录:`email`/`password`/`client`/`_csrf`;reset:`email`/`_csrf`;update:`resetToken`/`currentPassword`/`password`/`_csrf`),合法组合全部通过;
...
| 服务端插件(本地 env + 条件注册 ×2 Hook + 3 端点 + team 作用域 + reset token 消费链 + strict schema + 专用限流 + 对接) | 2.5 – 3.5 天 |
| CLI 脚本 + yarn script | 0.5 天 |
| 前端(登录分支 native form + 忘记密码子状态 + `/reset-password` 公开页 + 回跳域名推导 + 个人设置卡片) | 1.5 – 2 天 |
| 测试(17 项,CSRF/schema 四子项)与安全打磨 | 1.5 – 2.5 天 |
