# Outline 邮箱 + 密码登录功能 — 技术方案 v3.9

> 目标:在自托管的 Outline(fork)中新增"邮箱 + 密码"登录方式。
> 状态:设计稿 v3.9(十一轮代码评审后,评审结论:可以落地,按本版执行)· 适用版本:outline/outline main 分支(2026 上半年)
> 前稿:`outline-password-auth-plan-v3.md`(v3 / v3.1)、`-v3.2.md` ~ `-v3.8.md`。本稿为独立全文,以本稿为准。
...
| # | 评审意见 | 处理 |
|---|---|---|
| 1(Medium) | v3.8 称带 `?token=` 的请求被空 query strict 400 且"无任何状态变更"——断言过强:`/auth` 的 provider router 统一挂载在 **`authMiddleware({ optional: true })`** 之后(`server/routes/auth/index.ts:26`),若 `?token=` 是**有效** JWT / API token,optional auth 会在 `validate()` 之前成功解析并刷新 `user.updateActiveAt` / `team.updateActiveAt`(`server/middlewares/authentication.ts:43`),API key 还会 `apiKey.updateActiveAt()`(`:241`)。空 query strict 能阻止 password handler 执行,但不能保证"全局无状态变更" | §5.0 query 向量的防线表述收窄为"**password 端点的状态变更不会执行**",并补 optional auth 副作用说明(含一处核对补充:OAuth access token 被强制 header transport,`:157`,query 传入会被拒、由 optional 吞掉——能触发 activeAt 副作用的是有效 JWT 与 API key 两类);说明该副作用是 `/auth` 全部 provider 路由的**既有行为**,非本功能引入,activeAt 刷新也非安全敏感状态;测试 14e 断言同步收窄为"handler 未执行 + 密码/失败计数/password 相关 events 未变化",**不断言全局无状态变化**;"`/auth` 全局层在 provider mount 前拦截 query token"作为彻底封堵选项移入 §10(影响现有 auth callback 设计,需单独评审) |

**v3.7 → v3.8(上轮,已收口)**:`token` 字段红线从 body 扩展到 query——`parseAuthentication` 取 token 顺序 body → query → cookie(`server/middlewares/authentication.ts:105` / `:114`),`?token=` 同样污染 transport 使 CSRF 被跳过;三个 schema 加 `query: z.object({}).strict()`(合法请求不带 query,该 400 是 query 向量的拒绝点;body 向量仍以改名 `resetToken` 为修复、strict 为纵深);update schema 以 `.superRefine()` 实现 `resetToken` / `currentPassword` 的 XOR + `.min(1)` 防空串;`/reset-password?token=` 页面 URL 保留(SPA GET 路由,不在红线范围)。

**v3.6 → v3.7(更早,已收口)**:`/auth/password/*` 的 body schema 必须 `.strict()` 并枚举全部合法字段(含 `_csrf`、`client`)——`validate()` 只是 `schema.parse(ctx.request)`(`server/middlewares/validate.ts:14`),Zod object 默认剥离未知字段不报错,`BaseSchema` 无全局 strict(`server/routes/api/schema.ts:9`);同时明确中间件执行顺序——`verifyCSRFToken` 先于 route validation,schema 拒绝只是纵深,不能替代 body 字段改名对 CSRF 的修复。
...
- **(v3.9)`/auth` 全局层拦截 query token**:空 query strict 只能在 route 层拒绝,挡不住前置 `authMiddleware({ optional: true })` 对有效 query token 的解析与 activeAt 副作用(§5.0)。若要彻底阻止,需在 `/auth` app 层、provider mount 之前加专门拦截(如对特定路径剥离/拒绝 query `token`)——但 optional auth 是所有 provider 路由共享的,部分 provider 的 OAuth callback 本身依赖 query 传参,改动影响现有 auth callback 设计,需单独评审。首版接受 activeAt 刷新这一非安全敏感副作用。
